...
if ! command -v $PKG &> /dev/null; then
  if command -v apt &> /dev/null; then
    ...
    echo "🟨 Need superuser password to install eza using Pacman package manager..."
    echo "🔹  sudo pacman -S --noconfirm eza"
    sudo pacman -S --noconfirm eza
  elif command -v dnf &> /dev/null; then
    echo "🟨 Need superuser password to install eza using DNF package manager..."
    echo "🔹  sudo dnf install -y eza"
