...
# Function to extract date from filename with DD_MM_YYYY or DD-MM-YYYY pattern
extract_date_from_filename_dd_mm_yyyy() {
    local filename="$1"
...
organize_files() {
    ...
    fd --full-path "$input_dir" -e jpg -e jpeg -e JPG -e JPEG -e png -e PNG -e gif -e GIF -e mp4 -e mov -e MP4 -e MOV | while read -r file; do
        ...
        if [ -z "$date_info" ]; then
            date_info=$(extract_date_from_filename_dd_mm_yyyy "$filename")
        fi
